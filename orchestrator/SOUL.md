...
├── state/
│   ├── 01_clarified.json   # 需求澄清
│   ├── 02_analyzed.json    # 需求分析
│   ├── 03_matched.json     # 经验匹配
│   ├── 04_planned.json     # 流程规划
│   ├── 05_designed.json    # 团队设计
│   └── 07_executed.json    # 执行完成
├── agents/                    # AI团队成员工作区
│   ├── {agent_id_1}/
│   │   ├── SOUL.md          # Agent个人SOUL
│   │   └── config.json      # Agent配置
...
| 文件 | 说明 |
|------|------|
| `teams/{team_id}/config.json` | 团队配置 |
| `teams/{team_id}/workflow.json` | 执行流程 |
| `teams/{team_id}/agents/{id}/SOUL.md` | 每个Agent的SOUL |
| `teams/{team_id}/agents/{id}/config.json` | 每个Agent的配置 |
...
```json
{
  ...
  "生成的文件": [
    ...
    "teams/{team_id}/workflow.json",
    "teams/{team_id}/agents/{id}/SOUL.md",
    "teams/{team_id}/agents/{id}/config.json"
  ],
  "完成时间": "ISO时间戳"
}
```
...
| 场景 | 动作 |
|------|------|
| 子任务完成 | supervisor 评分 → ≥80 直接过 | 60-80 记录改进点继续 | <60 回退 |
...
|----------|-----------|------|
| agent-compiler | Agent编译器 | 编译和生成 Agent 的配置 |
| agent-prompt-designer | Agent提示词设计师 | 设计 Agent 的 SOUL 内容 |

### 6.2 可选 Skill

| Skill ID | Skill 名称 | 说明 |
|----------|-----------|------|
| skill-matcher | 技能匹配器 | 确认 Agent 技能配置 |
